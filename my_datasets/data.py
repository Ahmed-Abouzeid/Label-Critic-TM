...
class Synthetic(object):
    ...
    def __init__(self, min_num_samples, max_num_samples, feat_fim, subpatterns_num):
        self.max_num_samples = max_num_samples
        self.min_num_samples = min_num_samples
        self.feat_dim = feat_fim
        self.samples = {}
        self.subpatterns_num = subpatterns_num
    
    def create_patterns(self):
        if self.feat_dim not in [20, 40, 60, 100, 200, 400, 600, 800, 2000, 4000, 6000, 8000, 10000]:
            print('CANNOT CREATE SYNTHETIC DATA WITH THE GIVEN NUMBER OF FEATURES PER PATTERN!')
            exit()
        else:
            all_patterns = []
            start_index = 0
            for r in range(self.subpatterns_num//2):  # we create n sub patterns in the dataset, later we can decide on how many classes that copnclude some of these sub patterns
    ...
    def load_data(self):
        ...
        for k in classes_samples.keys():
            for sample in classes_samples[k]:
                X.append(sample)
                Y.append(k)
        return train_test_split(np.array(X), np.array(Y), random_state=42, shuffle=True, test_size=0.001), all_patterns
